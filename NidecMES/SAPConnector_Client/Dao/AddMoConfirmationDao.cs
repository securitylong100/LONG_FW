...
namespace Com.Nidec.Mes.SAPConnector_Client.Dao
{
    class AddMoConfirmationDao : AbstractSAPDataAccessObject
    {
        public override ValueObject Execute(TransactionContext trxContext, ValueObject vo)
        {
            ...
            sapParameter.AddParameter("PLANT", header.PlantCode);
            sapParameter.AddParameter("ORDER_NUMBER", header.OrderNumber);
            sapParameter.AddParameter("IMUSER", header.SapUserCode);
            sapParameter.AddParameter("IMDATE", header.RegistrationDateTime.ToString("yyyyMMdd"));
            sapParameter.AddParameter("IMTIME", header.RegistrationDateTime.ToString("HHmmss"));
            sapParameter.AddParameter("SUPERVISOR", header.Supervisor);
            ...
            foreach (MoConfirmationMaterialVo m in materials)
            {
                ...
                sapParameterTable.AddParameter("IMDATE", m.RegistrationDateTime.ToString("yyyyMMdd"));
                sapParameterTable.AddParameter("IMTIME", m.RegistrationDateTime.ToString("HHmmss"));
