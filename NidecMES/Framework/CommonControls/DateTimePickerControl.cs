...
namespace Com.Nidec.Mes.Framework.CommonControls
{
    public partial class DateTimePickerControl : UserControl
    {
        ...
        private void TimeValueChange(DateTime? datetime)
        {
            ...
            if (InputType == InputTypeList.ToDate)
            {
                Time_dtp.Value = new DateTime(Date_dtp.Value.Year, Date_dtp.Value.Month, Date_dtp.Value.Day, 23, 59, 59);
            }
            else if (InputType == InputTypeList.FromDate)
            {
                Time_dtp.Value = new DateTime(Date_dtp.Value.Year, Date_dtp.Value.Month, Date_dtp.Value.Day, 0, 0, 0);
            }
            else if (InputType == InputTypeList.NowWithZeroMin)
            {
                Time_dtp.Value = new DateTime(Date_dtp.Value.Year, Date_dtp.Value.Month, Date_dtp.Value.Day, Date_dtp.Value.Hour, 0, 0);
            }
            else
            {
                Time_dtp.Value = new DateTime(Date_dtp.Value.Year, Date_dtp.Value.Month, Date_dtp.Value.Day, Date_dtp.Value.Hour, Date_dtp.Value.Minute, 0);
